...
export const Route = createFileRoute('/admin/posts/$id')({
  loader: async ({ params }) => {
    return await getEditPostDataFn({ data: { id: params.id } });
  },
  component: AdminEditPost
})

function AdminEditPost() {
  const { post, categories: catList, tags: tagList, activeTagNames } = Route.useLoaderData() as { post: any; categories: any[]; tags: any[]; activeTagNames: string[] };
  ...
  const [categoryId, setCategoryId] = useState(post.categoryId || "__none__");
  const [newTagName, setNewTagName] = useState("");
  const [selectedTagNames, setSelectedTagNames] = useState<string[]>(activeTagNames || []);
  ...
  const handleAIAllInOne = async () => {
    ...
    try {
      ...
      // 2. Generate slug based on first title
      const firstTitle = titles[0] || "新文章";
      const generatedSlug = await aiGenerateSlugFn({ data: { title: firstTitle, postId: post.id } });
  ...
  const handleAISlug = async () => {
    ...
    try {
      const generated = await aiGenerateSlugFn({ data: { title, postId: post.id } });
      setSlug(generated);
  ...
  const handleSave = async (status: 'draft' | 'published') => {
    ...
    try {
      ...
      if (!finalSlug) {
        finalSlug = await aiGenerateSlugFn({ data: { title: title || "untitled", postId: post.id } });
      }
      ...
      toast.success(status === 'published' ? "文章已更新并发布" : "草稿已保存");
      ...
      <div className="shrink-0 border-b border-border/80 px-6 py-4 bg-background/80 backdrop-blur-md flex items-center justify-between z-20 shadow-sm">
        <div className="min-w-0 flex items-center gap-3">
          ...
          <div className="min-w-0">
            <h1 className="text-sm font-bold tracking-tight text-foreground/90 truncate">编辑文章</h1>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2 shrink-0">
          ...
          <Button type="button" size="sm" disabled={saving} onClick={() => handleSave('published')} className="text-xs h-8 bg-gradient-to-r from-primary to-orange-500 hover:from-primary/90 hover:to-orange-500/90 text-white border-0 shadow-sm transition-all hover:shadow-md">
            {saving ? (<><Loader2 size={13} className="animate-spin" /> 保存中…</>) : "更新并发布"}
          </Button>
        </div>
      </div>
          ...
          <div className="flex flex-col gap-2">
            ...
            <div className="rounded-lg border border-border bg-card overflow-hidden min-h-[400px]">
              <Suspense fallback={<div className="h-64 flex items-center justify-center text-sm text-muted-foreground">正在加载编辑器…</div>}>
                <Editor initialContent={post.contentBlocks} onChange={handleEditorChange} />
              </Suspense>
            </div>
          </div>
        </div>
