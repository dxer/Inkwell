...
export const getAdminPostsFn = createServerFn({ method: 'GET' })
  ...
  .handler(async ({ data }) => {
    ...
    const list = await query.orderBy(desc(posts.updatedAt), desc(posts.createdAt));
...
function AdminPostsIndex() {
  const { posts: list, categories: catList, tags: tagList, filters } = Route.useLoaderData() as { posts: any[]; categories: any[]; tags: any[]; filters: any };
